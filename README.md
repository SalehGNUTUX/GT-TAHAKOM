...
- **اكتشاف ذكي للأجهزة:** تلقائي كامل للأجهزة الشبكية (mDNS/SSDP)، وشبه آلي لأجهزة IR. التفاصيل في [docs/DISCOVERY.md](docs/DISCOVERY.md).
...
- **م0 (التأسيس) ✅** — مشروع Kotlin/Compose، طبقة `Transport` المجرّدة، النموذج الموحّد، `IrTransport`، Hilt، أيقونة، توثيق. **يبني APK بنجاح.**
- مزايا منجزة مبكراً: تبديل اللغة، صيغة `.tahakom` + الفتح المباشر، نموذج الاكتشاف الذكي.
- **التالي: م1** — تنفيذ الاكتشاف الفعلي (NSD/mDNS + SSDP) وقائمة الأجهزة.
