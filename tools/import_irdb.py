...
NAME_MAP = {
    ...
    "CURSOR UP": "NAV_UP", "CURSOR DOWN": "NAV_DOWN", "CURSOR LEFT": "NAV_LEFT", "CURSOR RIGHT": "NAV_RIGHT",
    "ARROW UP": "NAV_UP", "ARROW DOWN": "NAV_DOWN", "ARROW LEFT": "NAV_LEFT", "ARROW RIGHT": "NAV_RIGHT",
    "OK": "NAV_OK", "ENTER": "NAV_OK", "SELECT": "NAV_OK",
    "CH LIST": "LIST", "CHANNEL LIST": "LIST", "C.C.": "CC", "CC": "CC",
...
PIONEER_FREQ = 40000 # Hz — Pioneer (بنية NEC على 40kHz)
...
def when_protocol(p, device, subdevice, function):
    ...
    if p == "PIONEER":
        return pioneer_to_pronto(device, subdevice, function)
    if p == "PROTON":
        return proton_to_pronto(device, function)
    return None


def pioneer_to_pronto(device, subdevice, function):
    """Pioneer: بنية NEC نفسها (D, S, F, ~F، مسافة-نبضة) لكن على حامل 40kHz."""
    carrier = round(1_000_000 / (PIONEER_FREQ * 0.241246))
    sub = (~device) & 0xFF if subdevice < 0 else subdevice & 0xFF
    bytes_ = [device & 0xFF, sub, function & 0xFF, (~function) & 0xFF]
    unit = round(562.5 / (carrier * 0.241246))
    pairs = [16 * unit, 8 * unit]
    for b in bytes_:
        for i in range(8):
            pairs += [unit, 3 * unit] if (b >> i) & 1 else [unit, unit]
    pairs += [unit, 39 * unit]
    n = len(pairs) // 2
    return " ".join(f"{w:04x}" for w in [0x0000, carrier, 0x0000, n] + pairs)


def proton_to_pronto(device, function):
    """Proton: {38k,500}، قائد 8/8، D:8 ثم فجوة (1,-8) ثم F:8 ثم توقّف (1,-40)، LSB."""
    u = 500
    durs = [8 * u, 8 * u]  # قائد
    for i in range(8):
        durs += [u, 3 * u] if (device >> i) & 1 else [u, u]
    durs += [u, 8 * u]  # فجوة منتصف الإطار
    for i in range(8):
        durs += [u, 3 * u] if (function >> i) & 1 else [u, u]
    durs += [u, 40 * u]  # توقّف + فاصل
...
def _freq_for(proto_upper):
    ...
    if proto_upper == "MITSUBISHI":
        return MITS_FREQ
    if proto_upper == "PIONEER":
        return PIONEER_FREQ
    return NEC_FREQ  # NEC/JVC/Denon/Proton = 38kHz
